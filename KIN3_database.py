...
def add_token(code, member_id, file_path = './esi_tokens.txt'):
	app, security, client = KIN3_Esi.esi_latest
...
def get_character_object(member, char_index = 0):
	...
	if len(characters_list) > char_index:
		return KIN3_Esi.eve_character(characters_list[char_index], member)

	return None

async def process_char_index(member, char_index, channel, display_name, auth_embed):
	...
	if not auth_count > 0:
		...
		eve_char_object = get_character_object(member, char_index)
